'''simople_traj.py
   ...
   Intended to make Atlas_v5 urdf move up and down as if doing a pullup,
   with kinematic chains originating from hands and a task trajectory concerning
   pelvis motion.
...
joint_list = ['l_arm_wry2', 'l_arm_wrx', 'l_arm_wry',
              'l_arm_elx', 'l_arm_ely',
              'l_arm_shx', 'l_arm_shz',
              'r_arm_wry2', 'r_arm_wrx', 'r_arm_wry',
              'r_arm_elx', 'r_arm_ely',
              'r_arm_shx', 'r_arm_shz',
...
class Trajectory():
    ...
    def __init__(self, node):
        ...
        self.chain_larm = KinematicChain(node, 'l_hand', 'pelvis', self.jointnames()) # chain from 'utorso' if only want 7dof arm
        self.chain_rarm = KinematicChain(node, 'r_hand', 'pelvis', self.jointnames()) # chain from 'utorso' if only want 7dof arm
        ...
        self.q_l = np.array([0.0, 0.505, 0.0,
                             -0.743, 1.027,
                             0.0, 0.662,
                             0.0, 0.0, 0.0]).reshape(-1,1)
        
        self.q_r = np.array([0.0, -0.505, 0.0,
                             0.743, -1.027,
                             0.0, -0.662,
                             0.0, 0.0, 0.0]).reshape(-1,1)
        
        (self.x, self.R, _, _) = self.chain_larm.fkin(self.q_l)
    ...
    def evaluate(self, t, dt):
        ...
        '''
        q = np.vstack((self.q_l[0:6], self.q_r, z7, z12)) # important: this q should be same order as joints presented in joint_list above
        #qdot = np.vstack((qdot_l[0:6], qdot_r, z7, z12))
        qdot = np.zeros(36).reshape(-1,1)
